...
async def echo(websocket, path):
    ...
    async for message in websocket:
        if message is None:
            continue
        # Here, it is checked whether the message is a byte array or a JSON string
        if isinstance(message, str):
            try:
                ...
                # Change the selected exercise function
            ...
            if current_exercise_function is not None:
                ...
                if len(lmList) != 0:
                    # Calculates the angle of the points in the landmarks and writes them on the image
                    # if -> legs, else -> limbs
                    if is_legs:
                    ...
                    # Performs interpolation for the left and right limbs to show percentage
                    per_right = np.interp(angle_right, (min_degree, max_degree), (100, 0))
                    per_left = np.interp(angle_left, (min_degree, max_degree), (100, 0))

                    # Properties of the bar rectangle on the image
                    bar_color_left = (0, 255, 0)
                    bar_color_right = (0, 255, 0)
                    # Starting coordinates of the bar rectangle for the left and right limbs
                    ...
                    # Ending coordinates of the bar rectangle for the left and right limbs
                    ...
                    # Percentage text on the bar rectangle for the left and right limbs
                    bar_text_x_left = int(w * 0.84)
                    bar_text_y_left = int(h * 0.15)
                    bar_text_x_right = int(w * 0.03)
                    bar_text_y_right = int(h * 0.15)
                    # Y-axis position of the top part of the bar inside the bar rectangle for the left and right limbs
                    ...
                    # Size of the square rectangle, set to 30% of the smaller dimension (width or height)
                    sqr_size = int(min(w, h) * 0.30)  # default: 0.20
                    # Padding of the square rectangle from the edges, set to 7.5% of the smaller dimension
                    sqr_padding = int(min(w, h) * 0.075)  # default 0.05
                    # Starting and ending coordinates of the square rectangle
                    ...
                    # Starting and ending coordinates of the square rectangle (top-left and bottom-right)
                    sqr_rect_start = (sqr_rect_start_x, sqr_rect_start_y)
                    sqr_rect_end = (sqr_rect_start_x + sqr_size, sqr_rect_start_y + sqr_size)
                    # Font size for the text inside the square rectangle
                    sqr_font_scale = sqr_size / 250 * 6  # default: x8
                    # Calculate the geometric center of the square rectangle
                    sqr_text_x = sqr_rect_start_x + sqr_size // 2
                    sqr_text_y = sqr_rect_start_y + sqr_size // 2
                    # Used to calculate the alignment and space occupied by the text inside the square rectangle
                    align_text = \
                    ...
                    # Coordinates of the text inside the square rectangle
                    text_x = sqr_rect_start_x + (sqr_size - sqr_text_width)
                    text_y = sqr_rect_start_y + (sqr_size + sqr_text_height) // 2

                    # Updates the counter and bar_color variables, adding +0.5 for each downward and upward movement
                    # Continues processing by checking if the left and/or right limb is active
                    if is_rightLimb:
                        if per_right == 100:
                            bar_color_right = (0, 255, 0)
                            if dir_right == 0:
                    ...
                    # The bar graphically shows the repetition count
                    # For the left limb, the graph is on the left side
                    if is_leftLimb:
                    ...
                    # For the right limb, the graph is on the right side
                    if is_rightLimb:
                    ...
                    # Draws the counter on the image to show the repetition count
...
start_server = websockets.serve(echo, "0.0.0.0", 443)
